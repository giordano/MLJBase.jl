...
function load_boston()
    df = disallowmissing!(CSV.read(joinpath(datadir, "Boston.csv"),
...
function load_reduced_ames()
    df = disallowmissing!(CSV.read(joinpath(datadir, "reduced_ames.csv"),
                                   copycols=true, categorical=true))
...
function load_ames()
    df = disallowmissing!(CSV.read(joinpath(datadir, "ames.csv"),
                                   copycols=true, categorical=true))
    df[:target] = exp.(df[:target])
    return SupervisedTask(verbosity=0, data=df,
                          target=:target,
...
function load_iris()
    df = disallowmissing!(CSV.read(joinpath(datadir, "iris.csv"), pool=true,
                                   copycols=true, categorical=true))
    return SupervisedTask(verbosity=0, data=df,
                          target=:target,
                          is_probabilistic=false)
...
function load_crabs()
    df = disallowmissing!(CSV.read(joinpath(datadir, "crabs.csv"), pool=true,
                                   copycols=true, categorical=true))
    return SupervisedTask(verbosity=0, data=df,
                          target=:sp,
                          ignore=[:sex, :index],
